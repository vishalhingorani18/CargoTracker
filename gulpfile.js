// Generated on 2017-06-14 using generator-jhipster 4.5.3
'use strict';
...
gulp.task('data', function () {
    return gulp.src(config.app + 'app/data/**')
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(changed(config.dist + 'app/data'))
        .pipe(gulp.dest(config.dist + 'app/data'))
        .pipe(gulp.dest(config.dist))
        .pipe(browserSync.reload({stream: true}));
});
...
gulp.task('assets:prod', ['images', 'data', 'styles', 'html', 'copy:swagger', 'copy:images'], build);
